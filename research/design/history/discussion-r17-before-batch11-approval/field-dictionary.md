# Complete application field dictionary — proposed revision 17

Batches 1 through 8 are approved. Batch 9 retains only `Calculation`; customer claim administration is outside the approved buyer-adviser scope. Later batches and the complete database design remain under review.
...
Constraints: unique(kind,object_id); typed target existence and owner trigger; no arbitrary table names; unique(id,owner_id);owner_id immutable;deferred relationship checks also run on target updates/deletes; Artifact kinds resolve only through the reviewed target registry; buyer-scope records cannot point to removed claim-administration types; inventory/coverage artifact kinds resolve only to public immutable inventory/review/report rows
