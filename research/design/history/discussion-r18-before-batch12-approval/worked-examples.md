...
The room preference remains a `CustomerRequirement`; the base sum insured and bonus assumptions are typed `Calculation.inputs`; applicable room, related-deduction and medicine rules remain `PolicyRule` records. `Calculation.operations` stores each hypothetical step. A comparison cannot silently reuse this result after changed customer facts, selected sum insured or policy rules.
...
`PolicyRule.body.table` gives the axes, quantity dimension and exact header meaning; `PolicyRuleTableCell.selectors` selects the procedure and amount axis. Cell geometry, continuation pages and footnotes remain linked through exact evidence. “Up to SI” is an expression referring to finite SI, not unlimited. The Star oral-chemotherapy footnote addresses with/without-hospitalization inclusion, while its defined-limit condition concerns hospitalization expenses. Both are retained; the interaction remains blocked if the applicable meaning is unresolved.

## Deductible and premium-debt comparison — references 14-02, 14-09, 11-05

Under a stipulated Star Gold ₹300,000 threshold, illustrative independently admissible amounts of ₹200,000 and ₹250,000 produce ₹150,000 threshold excess. This is buyer-facing hypothetical arithmetic for explaining the product, not a stored customer bill or claim outcome. A base insurer's refusal does not itself define another policy's admissibility rule.

Separately, an illustration with ₹800,000 loss and ₹400,000 already received cannot represent ₹900,000 total recovery merely because another policy has a ₹500,000 threshold result. Each contract's conditions and remaining loss constrain the comparison. In another bounded example, a ₹100,000 benefit and ₹30,000 unpaid future instalments can illustrate ₹70,000 remittance under Star V.12's acceleration/recovery provision. Premium recovery is not an extra disease co-pay.

`Calculation.inputs` labels all amounts as hypothetical or customer-reported, and `Calculation.operations` preserves their distinct roles and order. CoverGuide does not turn the illustration into an actual claim or usage ledger.

## Unresolved operation order — reference 11-02

For a hypothetical ₹100,000 admitted amount, ₹10,000 deductible and 20% co-pay: deductible then co-pay gives ₹72,000; co-pay then deductible gives ₹70,000. Both intermediate results are arithmetically valid. Without applicable ordering evidence, the adviser cannot choose one as entitlement. Store two conditional calculation traces, retain the relevant rules as blocked and record the missing-ordering finding in the later general review record. The displayed answer may explain the difference and request the decisive clause.
...
## Claim procedures compared before purchase

Public claim-notification, document and settlement conditions remain PolicyRule records and may be compared as product features. CoverGuide does not create customer claim events, receipt records or payment records.
