...
This is a proposed representation, not an implemented evaluator. The companion [JSON Schema](json-contracts.schema.json) defines 52 named, closed structures using JSON Schema 2020-12. Every `json:Name` field in the dictionary resolves to one of them. Rule text, SQL and Python expressions are never executed from stored content. Examples containing customer data are synthetic.
