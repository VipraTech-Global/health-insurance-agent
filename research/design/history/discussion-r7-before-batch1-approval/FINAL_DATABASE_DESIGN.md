# CoverGuide — final proposed database design, revision 7

**Decision requested: detailed design discussion. Status: proposed, not approved or implemented.** This is the consolidated database proposal, including the field catalogue, relationships, conditional-rule contracts, retrieval operations, lifecycle and migration mapping. It incorporates the current assessment of all 400 cases and nine additional original-derived cases.
...
flowchart LR
    ...
    C --> P
    F[Attributed customer facts and corrections] --> S[Immutable fact snapshot]
...
| What does every field mean? | [Readable field dictionary](field-dictionary.md), [typed dictionary](entity-field-dictionary.json), [field traceability](field-traceability-r7.json) |
...
| What was checked for this revision? | [Current verification](../reports/final-design-verification-r7.json) and [package manifest](../reports/final-design-manifest-r7.json) |

The typed dictionary governs field definitions; the readable dictionary mirrors it. Rule-schema files govern JSON shape, while the value and lifecycle contracts govern cross-record meaning. Entity constraints govern relationships more narrowly than common defaults. Any remaining contradiction blocks the affected implementation until corrected; an implementer must not choose whichever definition is easier. Historical files retain their original revision and are not implicitly reissued as revision 7.

## Records that support the actual customer decision

| Information boundary | Proposed records and behavior |
|---|---|
| Buyer versus insured | Account owns the conversation. Person and Relationship identify the buyer, insured, parent, spouse, child and other relevant roles. A 29-year-old buyer asking for a 63-year-old parent never supplies the parent's eligibility age. |
| Facts and corrections | FactAssertion records subject, predicate, provenance, certainty, valid time and correction lineage. FactPredicate specifies type and cardinality. FactSnapshot and SnapshotFact seal the exact accepted assertions used by a turn. An interpretation or suggestion is not silently accepted. |
...
| Original versus interpretation | OriginalBlob preserves bytes. Acquisition records success or failure; SourceLinkObservation retains the label and link context. DocumentRevision, DocumentPage and EvidenceSpan identify the actual source and location. A filename, successful download or matching UIN does not prove applicability. |
...
| Price/provider observation versus permanent rule | Quote and components belong to an owner, snapshot and configuration. ProviderObservation identifies branch, insurer, service, observation time and validity. A stale quote cannot establish affordability; an incomplete hospital search cannot establish absence. |
...
| Preserved corpus | 2,942 object hashes verified: 2,466 PDFs and 476 non-PDFs. Current acquisition ledger: 2,725 acquired and 697 failed attempts | Exact bytes and failures are preserved. Acquisition totals do not measure relevant knowledge coverage. |
