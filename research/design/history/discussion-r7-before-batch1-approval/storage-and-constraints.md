# Physical storage and enforcement contract — proposed revision 7
...
External documents are untrusted data. Acquisition validates the exact HTTPS host, each redirect and resolved address under the existing [fetch boundary](../../docs/SECURITY.md); detected media type controls the reader. Parser/OCR workers run in a separate restricted process/container with read-only access to the selected input, an encrypted ephemeral output directory, no application/benchmark database credentials, no host mounts or unrelated customer originals, and denied network egress. Fetching is a separate permitted acquisition activity. Pin reader/model assets and a versioned finite resource profile covering bytes, page/pixel expansion, CPU/wall time and memory. A missing profile, unsupported format, limit hit or parser failure produces explicit failed/incomplete work; it cannot silently skip pages and publish a complete document.
...
| Membership overlap | All mutations lock affected ContractRevision parents FOR UPDATE before writing, in deterministic ID order. READ COMMITTED validation uses a fresh post-lock statement. Reject overlapping verified membership for the same revision/person and reject empty intervals. |
| Payment corrections and reversals | All writes lock the exact Claim parent before payment-set reads/writes. One current assertion per logical key; corrections share that key and increment revision. Reversals are separate actual transactions with evidenced sender/recipient, compatible owner/claim/currency and a bounded net balance. |
