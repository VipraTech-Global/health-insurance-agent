...
This guide explains all 98 application entities and the separate 36-entity evaluation database. The [complete field dictionary](field-dictionary.md) remains the authority for all 1,011 application fields, their types, defaults and validation.
...
- one `Person` for Asha;
- one `Person` for Ravi;
- one `Relationship` saying Ravi is Asha's father;
...
The first statement about diabetes is stored as a `FactAssertion` sourced from Asha's first message. The clarification creates new assertions:
...
The old statement is not erased. It is linked to its correction. A `FactSnapshot` then records exactly which accepted facts are current for the next answer.
...
- `TermsRevision`: the exact edition of the legal terms;
- `Configuration`: the selected variant and combination;
- `ConfigurationOption`: a rider, add-on or election.
...
Each statement is stored as a `Rule` with `RuleEvidence`. `RuleDependency` links the rules that must be read together. If two originals conflict, `RuleIssue` preserves the conflict and blocks an unconditional conclusion.
...
An actual issued offer becomes a `PolicyContract` and `ContractRevision`. Ravi becomes a `ContractMember` only when membership is evidenced. His personal disease co-pay becomes an `IndividualTerm`. General product wording cannot overwrite it.
...
Each policy statement has a `ClaimCitation` pointing to an exact `EvidenceSpan`. The decision also records the fact snapshot, product terms, rules, calculations and unresolved information it used.
...
|---|---|---|
| `Account` | The login that owns private information | Asha's account |
| `Person` | One human described by that account | Asha and Ravi are separate people |
| `Relationship` | A dated relationship between people | Ravi is Asha's father |
...
| `DecisionIntent` | What the customer is trying to achieve | Buy new cover for Ravi with a stated budget |
...
## Part 2: facts, corrections and snapshots
...
| Record | Simple meaning | Example |
|---|---|---|
| `FactPredicate` | The definition of a fact type | “diagnosis,” “residence,” or “intended insured” |
| `FactAssertion` | One statement with subject, source and certainty | “Ravi takes diabetes tablets,” reported by Asha |
| `FactSnapshot` | The exact set of facts accepted at one conversation revision | Facts used for answer revision 4 |
| `SnapshotFact` | A link saying how one assertion participates in a snapshot | Accepted, rejected or retained as a conflict |
| `ConversationReuseGrant` | Permission to reuse selected facts in another conversation | Reuse Ravi's age, but not an unrelated disclosure |
...
If a customer first says “one blood-pressure tablet” and later says “maybe two,” the snapshot can retain a conflict. It must not choose the newer message merely because it is newer if the customer has not actually corrected the first statement.
...
| Record | Simple meaning | Example |
|---|---|---|
| `SourceLocator` | A URL or other approved location | An insurer download URL |
| `SourceLinkObservation` | Where and how a link was seen | A row labelled “Product CIS” linked to a PDF |
| `Acquisition` | One fetch attempt | Failed on Monday, succeeded on Tuesday |
| `OriginalBlob` | Exact preserved bytes identified by their hash | The PDF actually received |
| `DocumentRevision` | What those bytes are understood to be | Policy wording, English, printed UIN X |
| `DocumentPage` | A physical page with dimensions and rotation | Physical page 7, printed page 5 |
| `EvidenceSpan` | The exact text/table area supporting a statement | Room-rent row plus header and footnote |
| `TermsDocument` | A document's role in one terms edition | This PDF is the contractual wording |

### Example: changed bytes at the same URL

Suppose `insurer.example/policy.pdf` produced file A in January and file B in June. CoverGuide stores two acquisitions and two exact byte hashes. It never overwrites A with B and calls B the historical document.
...
This is why `EvidenceSpan`, `RuleTable` and `RuleTableCell` are separate records.
...
| `TermsRevision` | One legal edition | Edition issued in 2026 |
| `Configuration` | One offered variant/combination | Gold, family floater, ₹10 lakh SI |
| `ConfigurationOption` | One selected option | Chronic-care add-on |

The database also retains:

| Record | Purpose |
|---|---|
| `LegacyListing` | The exact old catalogue label being reconciled |
| `ListingCandidate` | A possible match, with each unresolved identity dimension preserved |
...
A policy begins in 2023. A 2024 amendment changes the maximum waiting period at renewal while preserving earlier continuity credit. CoverGuide creates a new `TermsRevision`. It selects the revision using the supported renewal rule and date. It does not reset continuity merely because a new document was downloaded.
...
| Record | Simple meaning | Example |
|---|---|---|
| `PolicyContract` | Stable identity of an owned or proposed policy relationship | Ravi's policy number and insurer |
| `ContractRevision` | One issued version over a defined interval | Renewal configuration valid this year |
| `ContractMember` | One actual insured member | Ravi covered from 1 October |
| `IndividualTerm` | A personal accepted modification | 20% diabetes co-pay for Ravi |
| `CoverageLayer` | A separately tracked amount layer | ₹5 lakh base plus ₹2 lakh enhancement |
| `ContinuityCredit` | Prior duration accepted for a person/amount/benefit | 24 months credited on the first ₹5 lakh |
| `PolicyEvent` | A dated policy event | Renewal due, request received, cancellation accepted |
| `PaymentScheduleItem` | One premium instalment obligation/status | Second instalment due 1 January |

### Example: enhancement has a different waiting clock

Suppose Ravi ports ₹5 lakh with 24 months of accepted continuity and increases cover by ₹2 lakh. The old ₹5 lakh and new ₹2 lakh become separate `CoverageLayer` records. Continuity may apply fully to the first layer and not to the enhancement. One `wait_completed = true` flag would be wrong.
...
| Record | Simple meaning |
|---|---|
| `Rule` | One reviewed condition, benefit, limit, exclusion or calculation instruction |
| `RuleEvidence` | Exact source passages supporting or restricting it |
| `RuleDependency` | Another rule that must be loaded with it |
| `RuleIssue` | A missing source, ambiguity or contradiction |
| `RuleTable` | A table and the meaning of its rows/columns |
| `RuleTableCell` | One selected table result with its headers and footnotes |
...
If a detailed wording says 36 months and its own summary says four years, CoverGuide stores both passages and a material `RuleIssue`. It does not pick the shorter period because it benefits the customer, or the longer one because it is safer.
...
## Part 9: treatment, bills, claims and payments

These records stop four different numbers from being collapsed into one:
...
| `ExpenseLine` | One billed service or item |
| `ExpenseAllocation` | How part of an expense belongs to a package, time or scope |
| `Claim` | A claim sent to one insurer under one contract revision |
| `ClaimLineAssessment` | That insurer's treatment of one expense line |
| `UsageEntry` | An append-only debit, reserve, release or reversal against a cover pool |
| `ClaimEvent` | Submission, receipt, document request, completion or payment event |
| `ClaimDocumentRequirement` | One required or disputed document and its authority |
| `ClaimDocumentReceipt` | Evidence that a particular recipient received a particular document |
| `ClaimPayment` | A real disbursement or reversal with indemnity, interest and other components separated |

### Detailed claim-clock example
...
| Record | Simple meaning |
|---|---|
| `Quote` | A dated private offer for exact people, facts and configuration |
| `QuoteComponent` | Base premium, loading, discount, tax or fee |
| `Provider` | Exact hospital organization and branch |
| `Clinician` | An identified doctor or practitioner |
| `ProviderObservation` | A dated network, exclusion, accessibility or service observation |
...
The user submits one message. CoverGuide stores the message, facts, snapshot, turn and outbox work before starting inference. If the browser disconnects, reconnecting reads saved `TurnEvent` rows after the last cursor. It does not send the model request again.

### Example: a correction arrives while a model is working

The old worker may finish, but before publishing it must recheck the expected fact revision. If the customer corrected a material fact, the result cannot publish as current. The new turn uses the corrected snapshot.
...
## Part 13: independent rule inventory and knowledge coverage
...
| Record | Simple meaning |
|---|---|
| `RuleInventoryItem` | One candidate occurrence found by independent original reading |
| `InventoryRevision` | A sealed inventory scope and counting method |
| `InventoryMembership` | Whether an occurrence is included, structural, excluded or unresolved |
| `InventoryLineage` | Why items were split, merged or treated as qualifiers |
| `RuleInventorySupport` | Independent judgment that a curated rule represents one counted item |
| `CoverageReport` | Numerator, denominator and measured/unmeasured status |
| `CoverageReportSupport` | Exact support records used for the historical numerator |
...
3. Remove or redact it from messages, snapshots, model payloads, search derivatives, caches and saved answers.
...
The 98 application entities are not 98 independent features. Most exist because two facts that look similar have different meaning or lifecycle.
...
The [entity catalogue](entity-catalogue.md) is a shorter index of all 98 entities. The [technical storage contract](storage-and-constraints.md) explains database types, locking, encryption and enforcement. The [final proposed design](FINAL_DATABASE_DESIGN.md) records the authoritative scope and approval boundary.
