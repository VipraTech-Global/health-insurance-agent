# Proposed entity and field dictionary — discussion revision 5
...
| subject_kind | enum required | none / None | Explicit subject identity independent of conversational context; person,conversation,contract,provider | person,conversation,contract,provider | No authentic private value inspected; see synthetic worked examples. | ["CUS-02"] |
...
| created_at | timestamptz required | transaction timestamp at insertion / instant | When this row was first recorded; UTC storage; immutable; not a source effective date | null | No authentic field value established in reviewed evidence; synthetic examples only in worked-examples.md. | ["OPS-06"] |
| insurer_id | fk:Insurer required | none / None | Issuing organization; Required | null | No authentic field value established in reviewed evidence; synthetic examples only in worked-examples.md. | ["INS-01"] |
...
Constraints: no unique name assumption; identity anchored to insurer original.
...
| created_at | timestamptz required | transaction timestamp at insertion / instant | When this row was first recorded; UTC storage; immutable; not a source effective date | null | No authentic field value established in reviewed evidence; synthetic examples only in worked-examples.md. | ["OPS-06"] |
| owner_id | fk:Account required | none / None | Account whose authorization governs this row; Composite owner foreign keys on private relationships; mixed rows require public/private scope check | null | Intentionally no real customer/authentication values inspected or included. | ["OPS-01"] |
| contract_id | fk:PolicyContract required | none / None | Policy relationship; Same owner | null | No authentic field value established in reviewed evidence; synthetic examples only in worked-examples.md. | ["INS-08", "INS-07"] |
| kind | enum required | none / None | Event type; issue,receipt,renewal_due,premium_paid,instalment_due,cancellation_notice,withdrawal_notice,port_request_received,port_information_received,group_exit,claim_notified | issue,receipt,renewal_due,premium_paid,instalment_due,cancellation_notice,withdrawal_notice,port_request_received,port_information_received,group_exit,claim_notified | No authentic field value established in reviewed evidence; synthetic examples only in worked-examples.md. | ["INS-08", "INS-07"] |
...
Constraints: at least one provenance source; port clocks use actual relevant receipt; unique(id,owner_id);owner_id immutable;deferred relationship checks also run on target updates/deletes; contract_id IS NULL OR owner_id IS NOT NULL; source_message_id IS NULL OR owner_id IS NOT NULL; related_event_id IS NULL OR owner_id IS NOT NULL.
...
| subject_kind | enum required | none / None | Required fact subject; person,conversation,contract,provider | person,conversation,contract,provider | Synthetic design fixture only; authentic personal value unavailable/uninspected. | ["CUS-02"] |
