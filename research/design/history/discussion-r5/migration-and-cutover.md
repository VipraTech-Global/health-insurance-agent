...
Legacy SourceObservation.fetched_at is retained with its original meaning in the exact archived source payload. The pilot ingestion path stamps it after download/storage, while batch discovery may stamp before individual fetches. It must not become Acquisition.started_at without separate per-attempt evidence. A historical record missing required attempt timing stays an archived, retrievable legacy observation; inventing chronology to satisfy a required field is forbidden.
