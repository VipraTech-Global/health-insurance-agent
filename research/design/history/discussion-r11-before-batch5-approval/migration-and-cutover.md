...
Session continuity requires the same supported password encodings, compatible signing/auth/session backend configuration, cookie scope and expiry behavior. Copying session rows is insufficient proof. ProductVariant transfer is handled securely outside reports; no secret values appear in this package. If compatibility cannot be retained, the departure returns for discussion before implementation/cutover.
