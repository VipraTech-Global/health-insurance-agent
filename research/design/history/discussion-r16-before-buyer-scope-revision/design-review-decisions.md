...
10. `UsageEntry` points to `CustomerPolicyRevision` for the base cover and may additionally point to a `CustomerPolicyFact` when a posting consumes a separately tracked additional-cover fact.
...
## Batch 9 — approved 2026-09-12

Scope: treatment events, itemized expenses, prospective or issued-policy coverage assessments and deterministic arithmetic.

Approved decisions:

1. Use five models: `TreatmentEpisode`, `TreatmentExpense`, `CoverageAssessment`, `Calculation` and `ExpenseCoverageResult`.
2. Rename `ExpenseLine` to `TreatmentExpense`; combine service dates into `service_period`, use clear bill descriptions and expense types, and retain exact private evidence.
3. Fold the former `ExpenseAllocation` table into the closed `TreatmentExpense.allocations` structure because allocations are owned entirely by one expense and are exceptional rather than independently managed records.
4. `TreatmentEpisode` belongs to one person and keeps treatment period, setting, exact customer fact references, information basis, evidence and append-only correction history.
5. Separate prospective coverage assessment from a real insurer-submitted claim. `CoverageAssessment` evaluates exactly one treatment against either one issued `CustomerPolicyRevision` or one `ProductVariant` plus prospective selection; the two context branches are mutually exclusive.
6. Every assessment pins the exact `CustomerProfileRevision`, date and `AdviceRequest`. Later customer corrections create new inputs and assessments rather than rewriting old advice.
7. `Calculation` belongs to the `AdviceRequest`, not to a final `Decision`, so deterministic arithmetic may run before an answer is assembled. It stores code-controlled calculation type, exact engine version, typed inputs, ordered operations, result, assumptions and completion status.
8. Remove generic expression duplication and reviewer-name fields from `Calculation`; each operation stores its expression, rule IDs, evidence IDs, units and rounding. Independent review belongs to later review records.
9. Replace `ClaimLineAssessment` with `ExpenseCoverageResult`. It keeps admissible amount, deductible contribution, cover consumption and expected payable amount distinct, and points to exact calculation steps instead of duplicating prose and citations.
10. Explicit zero, unknown, unlimited and not-applicable quantity states remain distinct. Missing material inputs block exact arithmetic.
11. Corrections to treatment or expense inputs append superseding records. Published assessments and calculations are immutable and retain the exact earlier inputs.
12. Actual claim submission, insurer events, required documents, payments and cumulative cover usage remain pending for Batch 10. Expected payable amount is never treated as evidence of actual payment.

## Batch 10 onward — pending

No Batch 10 or later entity or field is approved yet. No Django models, migrations or schema-dependent implementation are authorized until the complete design is explicitly approved.
