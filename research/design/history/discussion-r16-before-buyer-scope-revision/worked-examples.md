...
`TreatmentExpense.allocations` preserve categories and affected portions; separate `CustomerPolicyFact` records retain base and bonus; rules select room base and medicine exception; `Calculation.operations` stores each step. A quote or coverage assessment cannot silently reuse this result after changed room availability, SI, bonus or bill items.
...
`ExpenseCoverageResult` separates admitted expense, deductible contribution, cover use and expected payable amount. The still-pending `UsageEntry` design references immutable coverage results, source events and unique posting keys. A correction must reverse affected entries under locks so retrying a worker cannot double-count them.
