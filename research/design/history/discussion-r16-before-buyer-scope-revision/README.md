# CoverGuide database proposal — revision 16 under staged review
...
| What does every field mean? | [Field traceability](field-traceability-r16.json), [physical storage/constraints](storage-and-constraints.md), [Readable field dictionary](field-dictionary.md), [complete typed dictionary with units, examples and traceability](entity-field-dictionary.json) |
...
| What has actually been verified? | [Current staged-design verification](../reports/final-design-verification-r16.json), [current source dependencies](../reports/source-completion-dependencies-r22.json), [database review](../reports/database-proposal-review-r6.json) |
...
erDiagram
    ...
    TreatmentEpisode ||--o{ TreatmentExpense : contains
    AdviceRequest ||--o{ CoverageAssessment : requests
    TreatmentEpisode ||--o{ CoverageAssessment : evaluates
    Calculation ||--o| CoverageAssessment : computes
    CoverageAssessment ||--o{ ExpenseCoverageResult : contains
    TreatmentExpense ||--o{ ExpenseCoverageResult : evaluates
    ExpenseCoverageResult ||--o{ UsageEntry : posts
```
...
| Rules and arithmetic | PolicyRule keeps typed conditions and effects. PolicyRuleEvidence preserves every required original passage; PolicyRuleLink loads definitions, exceptions and calculation inputs; PolicyRuleTableCell stores indexed table results while table axes remain in the rule body. A blocked rule and contradictory evidence prevent unsupported use. TreatmentEpisode and TreatmentExpense preserve the medical event and itemized bill. CoverageAssessment separates an issued-policy assessment from a prospective selection; ExpenseCoverageResult separates admissibility, deductible contribution, cover consumption and expected payable amount. Calculation keeps exact typed inputs, ordered operations, assumptions and results. Actual claim administration and cumulative usage remain pending Batch 10. |
...
| Processing and operations | Turn, Outbox and TurnEvent make work durable before dispatch and reconnectable without another inference. ModelRoute/Qualification/Attempt expose actual model identity and failure. ProcessingJob and ReviewRecord keep recovery and independent review explicit. Consent, deletion, holds and audit records retain authorized lifecycle evidence. Native Django authentication/session tables and LegacyMapping support preservation. |
