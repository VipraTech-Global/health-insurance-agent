...
Create one owned `Person` for buyer and another for father. The buyer's reported age and residence assertions retain their source turn. “This cover is for my father” creates intended-insured membership for father; it does not copy buyer age or health to him. A later correction to father's diabetes status creates a new father-subject assertion superseding the earlier father assertion. The buyer's medical history stays separate. A sealed snapshot selects current assertions. A running turn on the previous snapshot cannot publish as current; `Dependency` identifies affected calculations and candidate findings.

The decisive records are `FactAssertion.subject_kind/person_id`, provenance, `supersedes_id`, `SnapshotFact`, `FactSnapshot.revision`, `Turn.expected_revision` and the conversation pointer. Reopening preserves these facts; a new conversation needs an explicit reuse grant before importing them.
...
`ExpenseLine/Allocation` preserve categories and affected portions; separate `CoverageLayer` records retain base and bonus; rules select room base and medicine exception; `Calculation.trace` stores each step. A quote or claim cannot silently reuse this result after changed room availability, SI, bonus or bill items.
...
A hypothetical accepted Gold migration credits 12 months to the retained ₹300,000 and zero to a ₹200,000 enhancement. Against the stipulated 12-month PED wait, remaining durations are zero and 12 months respectively. Two `CoverageLayer` records and separate `ContinuityCredit` entries prevent a single waited flag from covering the enhancement.
...
A quote obtained for the wrong residence is bound to the old fact snapshot; a later correct residence invalidates that price comparison. Unknown tax/fees remain unknown, not zero. Loading applies to the evidenced component base, not automatically base-plus-addon.

If the customer erases one medical disclosure, the system traces its assertion and content copies, fences older jobs, removes/redacts dependent payloads and marks affected snapshots/answers nonreplayable. Unrelated facts can survive. A private original containing the disclosure is erased as an original; a retained authorized redacted derivative has a new hash. Historical citation unavailability is explicit. Backup restoration must apply deletion tombstones before serving data.
