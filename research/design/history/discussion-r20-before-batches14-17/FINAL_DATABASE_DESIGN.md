# CoverGuide — database design, revision 20 under staged review

**Approval status: Batches 1 and 2 were explicitly approved by the user on 2026-09-11, and Batches 3 through 13 were explicitly approved on 2026-09-12. Four remaining batches and the complete design are still under review; nothing has been implemented.** This is the consolidated database proposal, including the field catalogue, relationships, conditional-rule contracts, retrieval operations, lifecycle and migration mapping. It incorporates the current assessment of all 400 cases and nine additional original-derived cases.
...
The application catalogue contains **77 entities, 686 fields, 206 typed foreign-key relationships and 32 declared JSON definitions**. The separately administered benchmark proposal contains **36 entities, 398 fields and 28 JSON contracts**. Independent rule-inventory and knowledge-coverage records are deferred to a separately isolated quality-store review. These are catalogue sizes, not measures of advice accuracy or insurance coverage.

The design separates customer assertions, public product terms, actual issued cover, preserved originals, conditional rules, observed prices/providers, calculations and saved decisions. Each has different ownership, uncertainty and time semantics. A saved answer pins the exact customer facts, product configuration, knowledge release, calculations and supporting original passages used to produce it.
...
flowchart LR
    ...
    I --> P[Reviewed knowledge publication]
    C --> P
    ...
    P --> R
    S --> R
    R --> D[Saved recommendation, statements and citations]
    D --> E[Durable conversation events]
    X[Correction, changed source, expiry or erasure] --> V[Typed freshness checks]
    V --> D
```
...
| What does every field mean? | [Readable field dictionary](field-dictionary.md), [typed dictionary](entity-field-dictionary.json), [field traceability](field-traceability-r20.json) |
...
| How are rules and arithmetic represented? | [Values and conditional rules](value-and-rule-contracts.md), [32 declared JSON definitions](json-contracts.schema.json), [worked record examples](worked-record-examples.json) |
...
| Public product versus issued policy | Product identifies the family; PolicyVersion identifies original-backed terms independently of UIN; ProductVariant and ProductOption identify available selections. CustomerUploadedDocument records a private customer-supplied file. CustomerPolicy, CustomerPolicyRevision, PolicyMember and CustomerPolicyOption identify actual issued cover without treating a public recommendation as a policy. CustomerPolicyFact stores sourced personal terms, additional cover and continuity in one typed model; PolicyEvent records the sourced policy timeline. |
...
| Grant versus restriction | PolicyRule stores typed conditions and effects; PolicyRuleEvidence preserves supporting, restricting and conflicting passages; PolicyRuleLink loads definitions, exceptions and calculation inputs; PolicyRuleTableCell stores exact table results. Blocked rules cannot support an answer. |
| Buyer facts versus policy rules | Prior claims or planned treatment relevant to buying remain sourced customer facts or requirements. Public claim procedures and restrictions remain policy rules. No customer claim, bill or payment ledger is stored. |
| Combi wrapper versus components | `TermsComponent` and the three private contract-bundle models remain unapproved. The remaining-model review recommends keeping one simplified public package-component model and removing private bundle administration from the buying adviser. |
| Price/provider observation versus permanent rule | Quote belongs to an owner, customer profile revision and product variant; its closed breakdown and payment schedule retain personal pricing details. ProviderNetworkSnapshot records source scope, date and completeness, while ProviderNetworkEntry identifies exact listed facility status. A stale quote cannot establish affordability; an incomplete hospital search cannot establish absence. |
| Saved answer versus current truth | Recommendation, PolicyCandidateAssessment, PolicyRequirementMatch, InformationNeed, RecommendationStatement, RecommendationCitation and Calculation retain the exact customer revision, knowledge release, candidates, requirement results, gaps, arithmetic and evidence used. A later correction can make the old answer stale without rewriting what it originally said. |
...
Turns persist idempotent work before dispatch. Workers own inference and durable events; SSE reconnect reads events after the saved cursor and does not repeat inference. Fact revision, lease/fencing generation, the pinned knowledge release and erasure generation are checked before publishing. Cancellation, retry, expired leases, model failure and insufficient evidence have distinct durable outcomes.

Published knowledge releases, snapshots, rules and recommendation evidence are immutable subject to authorized erasure. A source update creates a new release; new recommendations use it after atomic publication, while old recommendations retain their original release. Deletion follows originals, extracts, prompts, copies, indexes, cached answers and late worker output; restore-time tombstones prevent resurrection. The later privacy batch will define the minimum deletion records without assuming a legal-hold subsystem.
