...
```mermaid
flowchart LR
    A[What the customer said] --> E[Recommendation process]
...
The answer is stored as a `Recommendation`. Each important statement becomes a `RecommendationStatement`, for example:
...
Each policy statement has a `RecommendationCitation` pointing to an exact `EvidenceSpan`. The decision also records the customer profile revision, product terms, rules, calculations and unresolved information it used.

If Asha later corrects Ravi's age, uploads an issued schedule or CoverGuide publishes a newly applicable amendment, the saved recommendation's direct links show which customer revision and knowledge release it used. The original answer remains in history but becomes stale for current use.
...
The upgraded room increases the customer's possible exposure by ₹44,000 in this illustration, even though the room itself costs only ₹6,000 more. `Calculation.operations` stores the hypothetical inputs and each policy-rule step so the comparison remains explainable without creating a customer treatment bill.

## Part 9: buyer-facing calculations and the claim boundary

CoverGuide stores public claim conditions as `PolicyRule` knowledge when those conditions help a customer compare policies. A customer statement such as “my father had two claims last year” becomes a sourced `CustomerFact` only when it matters to buying, renewal or portability.

`Calculation` stores exact arithmetic used for premium comparisons, ages, waiting-period dates, deductibles, percentages, limits and rankings. It keeps the inputs, operation order, units, rounding, assumptions and result.

CoverGuide does not store treatment bills, submitted claims, insurer receipts, document checklists, claim payments or remaining-cover ledgers. Those records belong to a claims-support product and are outside the approved buying-adviser scope.

### Example

A customer asks which policy has the shorter remaining waiting period for their father. The father’s condition and coverage-history dates are CustomerFact records. Each policy’s waiting-period rule is a PolicyRule. Calculation determines the remaining period for each candidate, and the later recommendation records explain the comparison with citations.
...
| Record | Simple meaning |
|---|---|
| `KnowledgeRelease` | One immutable published knowledge set |
| `KnowledgeReleaseRule` | Exact term/rule revisions inside that set |
| `KnowledgeChannel` | Which published set is currently active |
| `PolicySearchChunk` | A replaceable search copy linked back to originals |
| `Recommendation` | The complete saved outcome for exact inputs |
| `PolicyCandidateAssessment` | One exact policy configuration that was recommended, retained as an alternative, excluded or left uncertain |
| `PolicyRequirementMatch` | How that policy performs against one customer requirement |
| `InformationNeed` | A missing fact, requirement or document confirmation the adviser should ask about |
| `RecommendationStatement` | One independently checkable statement in the outcome |
| `RecommendationCitation` | Exact original evidence for one statement |
...
Suppose recommendation R used policy version T1. A newly acquired amendment creates T2 and a review finds it applicable to the customer's renewal date.

- R remains a historical record of what was answered using T1.
- Its exact knowledge-release and rule links preserve that it used T1.
- Current validity becomes stale or blocked.
- A new answer uses T2 after publication.
- CoverGuide never edits R to pretend it originally used T2.
...
The later processing review will decide whether a separate typed review record is necessary and what exact object it reviews.
...
The later privacy review will define the smallest typed record needed to erase private copies from prompts, indexes, caches and exports.
...
2. Follow the approved typed private-copy and recommendation links to every affected prompt, index, cache and export.
...
## Recommendations to make during your review
