# CoverGuide database proposal — revision 20 under staged review
...
The application proposal currently contains **77 entities, 686 fields, 206 typed foreign-key relationships and 32 declared JSON definitions**. Batches 1 through 13 are approved; Batch 9 now retains only `Calculation`, and customer claim administration is explicitly outside scope. Later batches and the complete design remain under review. A separate benchmark-store proposal contains 36 entities, 398 fields and 28 payload structures. The independent rule inventory and coverage measurement are also deferred to a separately isolated quality-store review. Those counts describe the proposed representation; they are not evidence of scenario success or complete policy knowledge.
...
| What does every field mean? | [Field traceability](field-traceability-r20.json), [physical storage/constraints](storage-and-constraints.md), [Readable field dictionary](field-dictionary.md), [complete typed dictionary with units, examples and traceability](entity-field-dictionary.json) |
...
| What has actually been verified? | [Current staged-design verification](../reports/final-design-verification-r20.json), [current source dependencies](../reports/source-completion-dependencies-r22.json), [database review](../reports/database-proposal-review-r6.json) |
...
erDiagram
    ...
    Turn ||--o{ Recommendation : produces
    Recommendation ||--o{ RecommendationStatement : explains
    RecommendationStatement ||--o{ RecommendationCitation : supports
    EvidenceSpan ||--o{ RecommendationCitation : resolves
    Insurer ||--o{ Product : offers
    Product ||--o{ PolicyVersion : versions
    PolicyVersion ||--o{ ProductVariant : selects
    ProductVariant ||--o{ ProductOption : offers
    PolicyVersion ||--o{ PolicyVersionDocument : requires
    ...
    SourceCapture |o--o{ EvidenceSpan : public_source
    CustomerUploadedDocument |o--o{ EvidenceSpan : private_source
    DocumentVersion ||--o{ PolicyVersionDocument : supplies
    PolicyVersion ||--o{ PolicyRule : contains
    PolicyRule ||--o{ PolicyRuleEvidence : supported_by
    EvidenceSpan ||--o{ PolicyRuleEvidence : supports
    PolicyRule ||--o{ PolicyRuleLink : connects
    PolicyRule ||--o{ PolicyRuleTableCell : selects
    CustomerPolicy ||--o{ CustomerPolicyRevision : records
    CustomerPolicyRevision ||--o{ PolicyMember : covers
    CustomerPolicyRevision ||--o{ CustomerPolicyOption : selects
    ProductVariant ||--o{ CustomerPolicyRevision : applies
    PolicyVersion ||--o{ TermsComponent : defines_slots
    ...
    CustomerPolicyRevision |o--o{ ContractBundleMember : supplies
    CustomerPolicy ||--o{ CustomerPolicyFact : partitions
```
...
| Originals and reconciliation | DiscoveryRun records one autonomous Codex search. SourceURL is one address, SourceObservation records each path by which Codex found it, SourceCapture is one preservation attempt and OriginalFile deduplicates exact bytes. DocumentSeries groups a continuing publication while DocumentVersion retains each edition and its dates. DocumentPage accounts for every PDF page and EvidenceSpan locates exact support. Old catalogue records remain migration evidence rather than live application models. |
| Product and issued cover | Product is a stable family, PolicyVersion is the applicable legal terms package, and PolicyVersionDocument links all governing originals. ProductVariant stores insurer-defined base choices and ProductOption stores available additions. CustomerPolicy and CustomerPolicyRevision describe actual owned cover; PolicyMember records covered people, CustomerPolicyFact records sourced personal terms, additional cover and continuity, and PolicyEvent records sourced policy history. |
...
| Rules and arithmetic | PolicyRule keeps typed conditions, effects, exceptions, calculation inputs and exact original support. Calculation keeps typed inputs, ordered operations, assumptions and results for buyer-facing comparisons. A prior claim or planned treatment remains a CustomerFact or CustomerRequirement; public claim conditions remain PolicyRule knowledge. Customer-specific claim administration is outside scope. |
| Changing observations | ProviderLocation identifies an exact facility branch. ProviderNetworkSnapshot preserves the dated insurer source, filters and completeness; ProviderNetworkEntry stores exact network, restricted or excluded rows. Quote belongs to an owner, customer profile revision and precise product selection, with a closed price breakdown and payment schedule. Observed prices and hospital status never become timeless policy rules. |
| Saved answers and retrieval | Recommendation pins the customer and knowledge revisions; PolicyCandidateAssessment and PolicyRequirementMatch store the comparison; InformationNeed stores focused gaps; RecommendationStatement and RecommendationCitation explain the result with exact evidence. KnowledgeRelease, KnowledgeReleaseRule and KnowledgeChannel publish the usable rule set, and PolicySearchChunk provides replaceable search text. |
| Processing and operations | Turn, Outbox and TurnEvent make work durable before dispatch and reconnectable without another inference. ModelRoute, ModelQualification and ModelAttempt expose actual model identity and failure. ProcessingJob keeps recovery explicit. Typed review records will be reviewed with processing; consent, deletion, holds and audit records remain later batches. Native Django authentication/session tables and LegacyMapping support preservation. |
