...
flowchart LR
    ...
    Q[Dated quotes and provider snapshots] --> C[Candidate comparison]
    P --> C
    R --> C
    C --> A[Recommendation statements]
...
A saved `Recommendation` identifies one `AdviceRequest`, final `CustomerProfileRevision`, published `KnowledgeRelease` and processing `Turn`. Each candidate has a `PolicyCandidateAssessment`; each customer condition has a `PolicyRequirementMatch`; missing material information becomes an `InformationNeed`; every customer-facing proposition is a `RecommendationStatement` with exact `RecommendationCitation` rows. `Calculation` stores typed inputs, ordered operations, assumptions and explicit unknown or blocked results.
