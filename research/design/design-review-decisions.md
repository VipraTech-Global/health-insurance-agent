...
Scope: durable customer turns, reconnectable events, reliable typed dispatch, strict model routing/call records and resumable policy-document processing.
...
6. Keep `ModelAttempt`; record exact identity, timing, usage and explicit failure, plus an optional encrypted response locator and digest. Exactly one turn or processing job is its parent.
7. Keep and correct `ProcessingJob`; exactly one public `SourceCapture` or private `CustomerUploadedDocument` is its input. Operational output uses a locator and digest and can never masquerade as contractual original evidence.
8. Independent review is a processing stage with explicit issues; no generic `ReviewRecord` is added.
9. These operational records support buying/comparison reliability and do not introduce claims or policy servicing.

## Batch 14 — approved by delegated review 2026-09-12

Scope: minimum privacy operations for a health-insurance buying adviser.

1. Keep simplified `ConsentRecord` only for privacy terms, health-data processing and document processing. Remove ABHA, external sharing, representation, recipient, transaction-key and guessed-expiry fields.
2. Keep simplified `DeletionRequest`; `created_at` is request time and closed verification proves completion across private stores.
3. Keep minimal `AuditEvent`; `created_at` is event time and metadata cannot contain medical text or secrets.
4. Remove/defer `RetentionHold`; no established buyer-adviser requirement justifies a legal-hold subsystem.

## Batch 15 — approved by delegated review 2026-09-12

Scope: Django framework and pilot migration boundary.

1. Remove `LegacyMapping` from the live application schema. Migration mappings and unsafe source archives remain immutable rehearsal/cutover artifacts.
2. Remove eight Django-managed auth, permission, session, content-type and admin tables from the custom CoverGuide entity catalogue. Preserve them through supported Django models and migrations, documented in `django-managed-tables.md`.

## Batch 16 — approved by delegated review 2026-09-12

Scope: public packaged/combi product composition needed for buying and comparison.

1. Rename `TermsComponent` to simplified `PolicyPackageComponent`.
2. Retain only the public, evidence-backed relationship between a package policy version and its separately issued required, optional or conditional component products.
3. Remove duplicate component self-supersession; package `PolicyVersion` history supplies versioning.
4. Remove `ContractBundle`, `ContractBundleRevision` and `ContractBundleMember`. Private package sealing and servicing are outside scope; existing component cover remains separate `CustomerPolicy` records and relevant package membership may be a sourced customer fact.

## Batch 17 — approved by delegated review 2026-09-12

Scope: physically separate independent quality and evaluation database.

1. Reduce the benchmark design from 36 models to 24 focused models.
2. Use external identity-provider subjects rather than duplicating benchmark users and roles.
3. Store protected payloads through typed storage-key/hash fields on their owning records rather than a generic artifact table.
4. Collapse case identity/version, assessment/version, attempt event/adjudication and mutable export wrappers while preserving immutable history and exact scoring.
5. Preserve the fixed 200 evaluation cases, 600 attempts, 60 robustness checks, 20 insurer inventories and every stated threshold.
6. Resolve the operational-family ambiguity: every family contains buyer/comparison decisions; operational behavior is judged as criteria within those cases.
7. Keep all case questions, oracles, expected sources, detailed responses and per-case results outside application storage, retrieval and implementation-readable infrastructure.

## Final critical audit — approved by delegated review 2026-09-12
