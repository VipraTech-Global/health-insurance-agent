    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=~Q(status__in=["completed", "failed", "cancelled"])
                | Q(completed_at__isnull=False),
                name="v2_discovery_run_ck_1",
            ),
            models.CheckConstraint(
                condition=~Q(status="failed")
                | (Q(error_summary__isnull=False) & ~Q(error_summary="")),
...
class SourceObservation(ApprovedModel):
    ...
    found_in_capture = models.ForeignKey(
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=~Q(disposition="irrelevant")
                | (Q(disposition_reason__isnull=False) & ~Q(disposition_reason="")),
...
class SourceCapture(ApprovedModel):
    ...
    status = models.CharField(
        max_length=16,
        choices=[("running", "running"), ("captured", "captured"), ("failed", "failed")],
        default="running",
    )
    http_status = models.SmallIntegerField(null=True, blank=True)
    original_file = models.ForeignKey(
        "OriginalFile", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    document_version = models.ForeignKey(
        "DocumentVersion", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    error_summary = models.TextField(null=True, blank=True)
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                | (Q(original_file__isnull=False) & Q(completed_at__isnull=False)),
                name="v2_source_capture_ck_2",
            ),
            models.CheckConstraint(
                condition=~Q(status="failed")
                | (Q(error_summary__isnull=False) & Q(completed_at__isnull=False)),
                name="v2_source_capture_ck_3",
            ),
            models.CheckConstraint(
                condition=~Q(status="running") | Q(completed_at__isnull=True),
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["owner", "sha256"], name="v2_original_file_uq_1", nulls_distinct=False
            ),
...
class DocumentSeries(ApprovedModel):
    ...
    issuer = models.ForeignKey(
        "Insurer", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
...
class DocumentVersion(ApprovedModel):
    ...
    document_series = models.ForeignKey(
        "DocumentSeries", on_delete=models.PROTECT, related_name="+"
    )
...
class EvidenceSpan(ApprovedModel):
    ...
    page = models.ForeignKey(
        "DocumentPage", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
