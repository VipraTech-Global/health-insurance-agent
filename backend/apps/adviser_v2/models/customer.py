...
class PersonRelationship(ApprovedModel):
    ...
    relationship_type = models.CharField(max_length=16, choices=[('spouse', 'spouse'), ('parent', 'parent'), ('child', 'child'), ('parent_in_law', 'parent_in_law'), ('sibling', 'sibling'), ('guardian', 'guardian'), ('dependent', 'dependent'), ('other', 'other')])
    source_statement = models.ForeignKey('CustomerStatement', on_delete=models.PROTECT, related_name='+')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_person_relationship_id_owner_uq'),
            models.UniqueConstraint(fields=['owner', 'from_person', 'to_person', 'relationship_type', 'valid_from', 'valid_to'], name='v2_person_relationship_uq_1', nulls_distinct=False),
            models.CheckConstraint(condition=~Q(from_person=F('to_person')), name='v2_person_relationship_ck_1'),
            models.CheckConstraint(condition=Q(valid_to__isnull=True) | Q(valid_from__isnull=True) | Q(valid_to__gte=F('valid_from')), name='v2_person_relationship_ck_2'),
        ]
...
class Conversation(ApprovedModel):
    ...
    current_profile_revision = models.ForeignKey('CustomerProfileRevision', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    status = models.CharField(max_length=16, choices=[('open', 'open'), ('archived', 'archived'), ('deleting', 'deleting'), ('deleted', 'deleted')], default='open')
...
class Message(ApprovedModel):
    ...
    role = models.CharField(max_length=16, choices=[('customer', 'customer'), ('adviser', 'adviser'), ('system', 'system')])
    content = EncryptedTextField()
    origin = models.CharField(max_length=19, choices=[('text', 'text'), ('voice_transcription', 'voice_transcription'), ('document_import', 'document_import'), ('system', 'system'), ('migration', 'migration')])
    client_request_id = models.UUIDField(null=True, blank=True)
    payload_commitment = models.CharField(max_length=64)
    submitted_at = models.DateTimeField()
    comparison = models.ForeignKey('Comparison', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(fields=['owner', 'client_request_id'], name='v2_message_uq_2', condition=Q(client_request_id__isnull=False)),
            models.UniqueConstraint(fields=['comparison'], name='v2_message_uq_3', condition=Q(comparison__isnull=False)),
...
class ConversationMessageChunk(ApprovedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_conversation_message_chunk_id_owner_uq'),
            models.UniqueConstraint(fields=['message', 'chunk_index', 'index_revision'], name='v2_conversation_message_chunk_uq_1'),
            models.CheckConstraint(condition=Q(chunk_index__gte=0), name='v2_conversation_message_chunk_ck_1'),
            models.CheckConstraint(condition=Q(start_offset__gte=0) & Q(end_offset__gt=F('start_offset')), name='v2_conversation_message_chunk_ck_2'),
        ]
        indexes = [
...
class CustomerStatement(ApprovedModel):
    ...
    kind = models.CharField(max_length=16, choices=[('fact', 'fact'), ('intended_insured', 'intended_insured'), ('requirement', 'requirement'), ('preference', 'preference'), ('question', 'question'), ('correction', 'correction'), ('context', 'context'), ('other', 'other')])
    status = models.CharField(max_length=22, choices=[('pending', 'pending'), ('mapped', 'mapped'), ('clarification_required', 'clarification_required'), ('ignored_with_reason', 'ignored_with_reason')], default='pending')
    resolution_note = EncryptedTextField(null=True, blank=True)

    class Meta:
        db_table = 'adviser_v2_customer_statement'
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_customer_statement_id_owner_uq'),
            models.CheckConstraint(condition=Q(start_offset__gte=0) & Q(end_offset__gt=F('start_offset')), name='v2_customer_statement_ck_1'),
        ]
...
class CustomerProfileRevision(ApprovedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_customer_profile_revision_id_owner_uq'),
            models.UniqueConstraint(fields=['conversation', 'revision'], name='v2_customer_profile_revision_uq_1'),
            models.CheckConstraint(condition=Q(revision__gt=0), name='v2_customer_profile_revision_ck_1'),
        ]
...
class CustomerFact(ApprovedModel):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='+')
    introduced_in_revision = models.ForeignKey('CustomerProfileRevision', on_delete=models.PROTECT, related_name='+')
    source_statement = models.ForeignKey('CustomerStatement', on_delete=models.PROTECT, related_name='+')
    ...
    status = models.CharField(max_length=16, choices=[('reported', 'reported'), ('confirmed', 'confirmed'), ('disputed', 'disputed'), ('retracted', 'retracted')], default='reported')
...
class CustomerRequirement(ApprovedModel):
    ...
    operator = models.CharField(max_length=21, choices=[('equals', 'equals'), ('not_equals', 'not_equals'), ('less_than_or_equal', 'less_than_or_equal'), ('greater_than_or_equal', 'greater_than_or_equal'), ('includes', 'includes'), ('excludes', 'excludes'), ('is_available', 'is_available'), ('is_not_available', 'is_not_available'), ('minimize', 'minimize'), ('maximize', 'maximize')])
    target_value = ValidatedJSONField(contract='FactValueV1', null=True, blank=True)
    priority = models.CharField(max_length=16, choices=[('mandatory', 'mandatory'), ('preferred', 'preferred'), ('informational', 'informational')])
    scope = models.CharField(max_length=20, choices=[('entire_purchase', 'entire_purchase'), ('all_intended_insured', 'all_intended_insured'), ('person', 'person')])
    subject_person = models.ForeignKey('Person', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    status = models.CharField(max_length=16, choices=[('reported', 'reported'), ('confirmed', 'confirmed'), ('disputed', 'disputed'), ('withdrawn', 'withdrawn')], default='reported')

    class Meta:
        db_table = 'adviser_v2_customer_requirement'
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_customer_requirement_id_owner_uq'),
        ]
        indexes = [
...
class AdviceRequest(ApprovedModel):
    ...
    conversation = models.ForeignKey('Conversation', on_delete=models.PROTECT, related_name='+')
    source_statement = models.ForeignKey('CustomerStatement', on_delete=models.PROTECT, related_name='+')
    request_type = models.CharField(max_length=18, choices=[('product_comparison', 'product_comparison'), ('coverage_question', 'coverage_question'), ('renewal_review', 'renewal_review'), ('portability_review', 'portability_review')])
    status = models.CharField(max_length=16, choices=[('open', 'open'), ('fulfilled', 'fulfilled'), ('cancelled', 'cancelled')], default='open')
...
class CustomerPolicy(ApprovedModel):
    ...
    proposer = models.ForeignKey('Person', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    payer = models.ForeignKey('Person', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    coverage_type = models.CharField(max_length=16, choices=[('individual', 'individual'), ('family_floater', 'family_floater'), ('group_member', 'group_member'), ('unresolved', 'unresolved')], default='unresolved')
    lifecycle_status = models.CharField(max_length=16, choices=[('offered', 'offered'), ('active', 'active'), ('lapsed', 'lapsed'), ('expired', 'expired'), ('cancelled', 'cancelled'), ('declined', 'declined'), ('unresolved', 'unresolved')], default='unresolved')
...
class CustomerPolicyRevision(ApprovedModel):
    ...
    updated_at = models.DateTimeField(auto_now=True)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='+')
    customer_policy = models.ForeignKey('CustomerPolicy', on_delete=models.PROTECT, related_name='+')
    ...
    selected_choices = ValidatedJSONField(contract='CustomerPolicySelectionV1', default=default_accepted_selection)
    selection_evidence = models.ForeignKey('EvidenceSpan', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    verification_status = models.CharField(max_length=16, choices=[('reported', 'reported'), ('verified', 'verified'), ('conflicted', 'conflicted')], default='reported')

    class Meta:
        db_table = 'adviser_v2_customer_policy_revision'
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_customer_policy_revision_id_owner_uq'),
            models.UniqueConstraint(fields=['customer_policy', 'revision_number'], name='v2_customer_policy_revision_uq_1'),
        ]
...
class PolicyMember(ApprovedModel):
    ...
    updated_at = models.DateTimeField(auto_now=True)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='+')
    customer_policy_revision = models.ForeignKey('CustomerPolicyRevision', on_delete=models.PROTECT, related_name='+')
    ...
    evidence_span = models.ForeignKey('EvidenceSpan', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    verification_status = models.CharField(max_length=16, choices=[('reported', 'reported'), ('verified', 'verified'), ('conflicted', 'conflicted')], default='reported')
...
class CustomerPolicyOption(ApprovedModel):
    ...
    selection_status = models.CharField(max_length=16, choices=[('selected', 'selected'), ('declined', 'declined'), ('unresolved', 'unresolved')], default='unresolved')
    evidence_span = models.ForeignKey('EvidenceSpan', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    verification_status = models.CharField(max_length=16, choices=[('reported', 'reported'), ('verified', 'verified'), ('conflicted', 'conflicted')], default='reported')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_customer_policy_option_id_owner_uq'),
            models.UniqueConstraint(fields=['customer_policy_revision', 'product_option'], name='v2_customer_policy_option_uq_1'),
        ]
...
class CustomerPolicyFact(ApprovedModel):
    ...
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='+')
    customer_policy_revision = models.ForeignKey('CustomerPolicyRevision', on_delete=models.PROTECT, related_name='+')
    person = models.ForeignKey('Person', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    fact_type = models.CharField(max_length=100)
    value = ValidatedJSONField(contract='CustomerPolicyFactValueV1')
    related_customer_policy = models.ForeignKey('CustomerPolicy', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    evidence_span = models.ForeignKey('EvidenceSpan', on_delete=models.PROTECT, related_name='+')
    verification_status = models.CharField(max_length=16, choices=[('extracted', 'extracted'), ('verified', 'verified'), ('conflicted', 'conflicted')], default='extracted')
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['id', 'owner'], name='v2_customer_policy_fact_id_owner_uq'),
            models.UniqueConstraint(fields=['supersedes'], name='v2_customer_policy_fact_uq_1', condition=Q(supersedes__isnull=False)),
        ]
...
class ConsentRecord(ApprovedModel):
    'One requested, granted or revoked processing-consent record; a requested row is not a grant.'
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    created_at = models.DateTimeField(auto_now_add=True)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='+')
    person = models.ForeignKey('Person', on_delete=models.PROTECT, related_name='+', null=True, blank=True)
    consent_type = models.CharField(max_length=22, choices=[('privacy_terms', 'privacy_terms'), ('health_data_processing', 'health_data_processing'), ('document_processing', 'document_processing')])
    notice_version = models.CharField(max_length=80)
    capture_method = models.CharField(max_length=17, choices=[('web_checkbox', 'web_checkbox'), ('conversation', 'conversation'), ('uploaded_document', 'uploaded_document'), ('migration', 'migration')])
    status = models.CharField(max_length=16, choices=[('requested', 'requested'), ('granted', 'granted'), ('revoked', 'revoked')], default='requested')
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(condition=(~Q(status='revoked') & Q(revoked_at__isnull=True)) | (Q(status='revoked') & Q(revoked_at__isnull=False)), name='v2_consent_record_ck_1'),
        ]
...
class DeletionRequest(ApprovedModel):
    ...
    state = models.CharField(max_length=16, choices=[('pending', 'pending'), ('running', 'running'), ('completed', 'completed'), ('failed', 'failed')], default='pending')
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(condition=~Q(state='completed') | Q(completed_at__isnull=False), name='v2_deletion_request_ck_1'),
            models.CheckConstraint(condition=~Q(state='failed') | Q(error_code__isnull=False), name='v2_deletion_request_ck_2'),
        ]
