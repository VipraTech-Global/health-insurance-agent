...
class Turn(ApprovedModel):
    ...
    starting_profile_revision = models.ForeignKey(
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                | (Q(lease_token__isnull=False) & Q(lease_until__isnull=False)),
                name="v2_turn_ck_1",
            ),
...
class Outbox(ApprovedModel):
    ...
    turn = models.ForeignKey(
        "Turn", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    processing_job = models.ForeignKey(
        "ProcessingJob", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=(
                    Q(turn__isnull=False)
                    & Q(processing_job__isnull=True)
                    & Q(knowledge_release__isnull=True)
                )
                | (
                    Q(turn__isnull=True)
                    & Q(processing_job__isnull=False)
                    & Q(knowledge_release__isnull=True)
                )
                | (
                    Q(turn__isnull=True)
                    & Q(processing_job__isnull=True)
                    & Q(knowledge_release__isnull=False)
                ),
                name="v2_outbox_ck_1",
            ),
            models.CheckConstraint(
                condition=~Q(state="leased") | Q(lease_until__isnull=False), name="v2_outbox_ck_2"
            ),
        ]
        indexes = [
...
class ModelAttempt(ApprovedModel):
    ...
    qualification = models.ForeignKey(
        "ModelQualification", on_delete=models.PROTECT, related_name="+"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=(Q(turn__isnull=False) & Q(processing_job__isnull=True))
                | (Q(turn__isnull=True) & Q(processing_job__isnull=False)),
            ...
            models.CheckConstraint(
                condition=(
                    Q(response_storage_key__isnull=True) & Q(response_storage_sha256__isnull=True)
                )
                | (
                    Q(response_storage_key__isnull=False) & Q(response_storage_sha256__isnull=False)
                ),
...
class ProcessingJob(ApprovedModel):
    "One resumable public-policy or private-upload processing stage."
    ...
    source_capture = models.ForeignKey(
        "SourceCapture", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    customer_uploaded_document = models.ForeignKey(
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["source_capture", "stage", "input_commitment", "attempt_number"],
                name="v2_processing_job_uq_1",
                condition=Q(source_capture__isnull=False),
            ),
            models.UniqueConstraint(
                fields=[
                ...
                condition=Q(customer_uploaded_document__isnull=False),
            ),
            models.CheckConstraint(
                condition=(
                    Q(source_capture__isnull=False) & Q(customer_uploaded_document__isnull=True)
                )
                | (Q(source_capture__isnull=True) & Q(customer_uploaded_document__isnull=False)),
                name="v2_processing_job_ck_1",
            ),
            models.CheckConstraint(
                condition=Q(attempt_number__gte=1) & Q(attempt_number__lte=3),
            ...
            models.CheckConstraint(
                condition=(
                    Q(result_storage_key__isnull=True) & Q(result_storage_sha256__isnull=True)
                )
                | (Q(result_storage_key__isnull=False) & Q(result_storage_sha256__isnull=False)),
