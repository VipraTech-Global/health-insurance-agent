...
class Product(ApprovedModel):
    ...
    recommendation_role = models.CharField(
        max_length=16,
        choices=[
    ...
    identity_evidence = models.ForeignKey(
        "EvidenceSpan", on_delete=models.PROTECT, related_name="+"
    )
...
class PolicyVersionDocument(ApprovedModel):
    ...
    applicability = ValidatedJSONField(contract="ApplicabilityV1")
    precedence_evidence = models.ForeignKey(
...
class ProductOption(ApprovedModel):
    ...
    option_policy_version = models.ForeignKey(
        "PolicyVersion", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
...
class PolicyRuleEvidence(ApprovedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["policy_rule", "evidence_span", "role"], name="v2_policy_rule_evidence_uq_1"
            ),
        ]
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=~Q(from_policy_rule=F("to_policy_rule")), name="v2_policy_rule_link_ck_1"
            ),
        ]
...
class PolicyRuleTableCell(ApprovedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["policy_rule", "selectors"], name="v2_policy_rule_table_cell_uq_1"
            ),
        ]
...
class ProviderNetworkSnapshot(ApprovedModel):
    ...
    scope_evidence = models.ForeignKey(
        "EvidenceSpan", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
...
class ProviderNetworkEntry(ApprovedModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    created_at = models.DateTimeField(auto_now_add=True)
    provider_network_snapshot = models.ForeignKey(
        "ProviderNetworkSnapshot", on_delete=models.PROTECT, related_name="+"
    )
    provider_location = models.ForeignKey(
        "ProviderLocation", on_delete=models.PROTECT, related_name="+"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["provider_network_snapshot", "provider_location", "network_status"],
...
class KnowledgeRelease(ApprovedModel):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(release_number__gt=0), name="v2_knowledge_release_ck_1"
            ),
            models.CheckConstraint(
                condition=(
                    Q(state__in=["draft", "ready", "blocked"]) & Q(published_at__isnull=True)
                )
                | (Q(state__in=["published", "retired"]) & Q(published_at__isnull=False)),
...
class KnowledgeReleaseRule(ApprovedModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["knowledge_release", "policy_rule"], name="v2_knowledge_release_rule_uq_1"
            ),
        ]
...
class KnowledgeChannel(ApprovedModel):
    ...
    current_release = models.ForeignKey(
        "KnowledgeRelease", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
...
class PolicyPackageComponent(ApprovedModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    created_at = models.DateTimeField(auto_now_add=True)
    package_policy_version = models.ForeignKey(
        "PolicyVersion", on_delete=models.PROTECT, related_name="+"
    )
    slot_key = models.CharField(max_length=80)
    component_product = models.ForeignKey(
        "Product", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    component_policy_version = models.ForeignKey(
        "PolicyVersion", on_delete=models.PROTECT, related_name="+", null=True, blank=True
    )
    ...
    selection_policy_rule = models.ForeignKey(
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=~Q(review_status="reviewed")
                | (Q(component_product__isnull=False) & Q(evidence_span__isnull=False)),
